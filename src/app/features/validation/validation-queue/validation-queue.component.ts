...
export class ValidationQueueComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.validationForm = this.fb.group({
      ...
      confidence: [0.8, [Validators.required, Validators.min(0.1), Validators.max(1.0)]],
  ...
  // Form getter
  get notes() { return this.validationForm.get('notes'); }
  ...
  loadPendingContributions(): void {
    ...
    this.contributionService.getPendingContributions(params).pipe(
  ...
  submitValidation(contribution: Contribution): void {
    ...
    const timeSpent = Math.round((Date.now() - this.validationStartTime) / 1000);
    ...
    this.contributionService.createValidation(contribution.id, validationData).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
      next: (response) => {
  ...
  quickValidation(contribution: Contribution, status: 'approved' | 'rejected'): void {
    this.isSubmittingValidation = true;
