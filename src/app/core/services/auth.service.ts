...
export class AuthService {
  ...
  public logout(): Observable<void> {
    // Only make the logout request if user is authenticated
  ...
  public requestPasswordReset(email: string): Observable<void> {
    return this.http.post<AuthResponse>(`${this.apiUrl}/request-password-reset`, {
      email
    }).pipe(
      map(response => {
        if (response.success) {
  ...
  public verifyEmail(token: string): Observable<void> {
    return this.http.get<AuthResponse>(`${this.apiUrl}/verify-email/${token}`).pipe(
      map(response => {
        if (response.success) {
