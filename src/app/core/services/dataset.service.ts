  ...
  return this.http.get<DatasetListResponse>(this.apiUrl, { params: httpParams }).pipe(
    map(response => {
      ...
      response.data.datasets = response.data.datasets.map(dataset => ({
        ...
        created_at: dataset.created_at ? new Date(dataset.created_at) : 
                   (dataset.createdAt ? new Date(dataset.createdAt) : new Date()),
        updated_at: dataset.updated_at ? new Date(dataset.updated_at) : 
                   (dataset.updatedAt ? new Date(dataset.updatedAt) : new Date())
      }));
